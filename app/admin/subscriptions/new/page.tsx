    ...
    try {
      ...
      const response = await fetch('/api/admin/subscription-links', {
        ...
        body: JSON.stringify({
          ...
          subscriptionDays: parseInt(formData.subscriptionDays)
        })
      })
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">
                  لینک بعد از این تعداد روز غیرفعال می‌شود (پیشنهاد: ۳۰ روز)
                </p>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData(prev => ({ ...prev, subscriptionDays: e.target.value }))}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  required
                />
                <p className="text-sm text-gray-600 mt-1">
                  هر کاربر چند روز اشتراک دریافت کند (پیشنهاد: ۱۸۰ روز = ۶ ماه)
                </p>
              </div>
              
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <h3 className="font-bold text-blue-900 mb-2">📝 خلاصه:</h3>
                <p className="text-sm text-blue-800">
                  لینک تا <strong>{formData.days} روز</strong> آینده فعال است و
                  <strong> {formData.maxRedemptions} </strong>نفر می‌توانند از آن استفاده کنند.
                  هر کاربر <strong>{formData.subscriptionDays} روز</strong> اشتراک فعال دریافت می‌کند.
                </p>
              </div>
