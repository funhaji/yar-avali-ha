...
import { GraduationCap, MapPin, Building2, ChevronRight, Phone } from 'lucide-react'
        ...
        <div className="bg-white md:rounded-2xl md:border md:border-line-soft overflow-hidden shadow-sm">
          ...
          <div className="relative h-48 md:h-56 bg-slate-800" style={{ background: 'linear-gradient(135deg, #1e293b 0%, #0f172a 100%)' }}>
            <div className="absolute top-4 right-4 z-10">
              <Link href="/teachers" className="inline-flex items-center gap-1 text-white/80 hover:text-white bg-black/20 hover:bg-black/40 px-3 py-1.5 rounded-lg backdrop-blur-sm transition-colors text-sm">
          ...
          <div className="px-6 md:px-12 pb-12 relative">
            ...
            {/* Details List */}
            <div className="space-y-4 mb-8 max-w-lg mx-auto md:mx-0 text-ink-soft">
              {teacher.education && (
            ...
            {/* Socials / Contacts */}
            {(teacher.contact_phone || teacher.telegram_id || teacher.whatsapp_id || teacher.eitaa_id || teacher.instagram_id) && (
              <div className="flex flex-wrap items-center gap-3 mb-10 justify-center md:justify-start">
                {teacher.contact_phone && (
                  <a href={`tel:${teacher.contact_phone}`} className="inline-flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-xl transition-colors font-bold text-sm" dir="ltr">
                    <Phone className="w-4 h-4" /> {teacher.contact_phone}
                  </a>
                )}
                {teacher.telegram_id && (
                  <a href={`https://t.me/${teacher.telegram_id.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-[#229ED9]/10 hover:bg-[#229ED9]/20 text-[#229ED9] px-4 py-2 rounded-xl transition-colors font-bold text-sm" dir="ltr">
                    @{teacher.telegram_id.replace('@', '')} تلگرام
                  </a>
                )}
                {teacher.whatsapp_id && (
                  <a href={`https://wa.me/${teacher.whatsapp_id.replace(/^0/, '98').replace(/\+/g, '')}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-[#25D366]/10 hover:bg-[#25D366]/20 text-[#25D366] px-4 py-2 rounded-xl transition-colors font-bold text-sm" dir="ltr">
                    {teacher.whatsapp_id} واتساپ
                  </a>
                )}
                {teacher.instagram_id && (
                  <a href={`https://instagram.com/${teacher.instagram_id.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-[#E1306C]/10 hover:bg-[#E1306C]/20 text-[#E1306C] px-4 py-2 rounded-xl transition-colors font-bold text-sm" dir="ltr">
                    @{teacher.instagram_id.replace('@', '')} اینستاگرام
                  </a>
                )}
                {teacher.eitaa_id && (
                  <a href={`https://eitaa.com/${teacher.eitaa_id.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-[#F26422]/10 hover:bg-[#F26422]/20 text-[#F26422] px-4 py-2 rounded-xl transition-colors font-bold text-sm" dir="ltr">
                    @{teacher.eitaa_id.replace('@', '')} ایتا
                  </a>
                )}
              </div>
            )}
